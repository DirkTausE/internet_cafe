...
// web/index.php
<<<<<<< HEAD
// Robust, minimal page that uses web/db.php helpers to show powered-on computers and customers.

declare(strict_types=1);

// for local debugging you can enable errors temporarily (remove in production)
// ini_set('display_errors','1'); ini_set('display_startup_errors','1'); error_reporting(E_ALL);

// Lade DB‑Helper (liefert fetch_computers(), fetch_customers(), db_get_pdo())
require_once __DIR__ . '/db.php';

// kleine Hilfsfunktionen
function esc(string $s): string { return htmlspecialchars($s, ENT_QUOTES, 'UTF-8'); }

function is_powered_on(array $c): bool {
    // unterstütze verschiedene Feldnamen/Typen
    if (array_key_exists('is_on', $c)) {
        if ($c['is_on'] === true || $c['is_on'] === 1 || $c['is_on'] === '1') return true;
    }
    foreach (['status','state','current_state','power'] as $k) {
        if (array_key_exists($k, $c) && $c[$k] !== null) {
            $v = strtolower((string)$c[$k]);
            if (in_array($v, ['on','online','up','powered-on','powered_on','frei','gast','running'], true)) return true;
        }
    }
    return false;
}

// Daten holen (db.php liefert leere Arrays bei Problemen)
$computers = fetch_computers();
$customers = fetch_customers();

// Filter nur eingeschaltete Rechner
$poweredOnComputers = array_values(array_filter($computers, function($computer){
    return is_powered_on((array)$computer);
}));
...
<head>
  ...
  <title><?= esc('Internetcafe — Übersicht') ?></title>
...
<body>
  <div class="container">
    <header>
      ...
      <p class="muted">Letzte Aktualisierung: <?= esc(date('Y-m-d H:i:s')) ?> — Auto-Refresh jede 30s.</p>
    </header>
